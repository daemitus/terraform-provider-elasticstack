		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("elasticstack_kibana_space.test_space", "id", spaceId),
					resource.TestCheckResourceAttr("elasticstack_kibana_space.test_space", "name", fmt.Sprintf("Created %s", spaceId)),
...
func checkResourceSpaceDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		res, diags := client.ReadSpace(ctx, rs.Primary.ID)
		if diags.HasError() {
			return fmt.Errorf(diags.Errors()[0].Summary())
		}

		if res != nil {
			return fmt.Errorf("space id=%v, but it should have been removed", rs.Primary.ID)
		}
	}
	return nil
}
