...
var ProviderFactories = map[string]func() (tfprotov6.ProviderServer, error){
	"elasticstack": providerserver.NewProtocol6WithError(
...
func PreCheck(t *testing.T) func() {
	return func() {
		for _, service := range []string{"ELASTICSEARCH", "KIBANA", "FLEET"} {
			for _, name := range []string{"ENDPOINT", "USERNAME", "PASSWORD"} {
				env := fmt.Sprintf("%s_%s", service, name)
				if val := os.Getenv(env); val == "" {
					t.Fatalf("%s must be set for acceptance tests to run", env)
				}
			}
		}
	}
...
// RandUUID generated a random UUIDv4.
