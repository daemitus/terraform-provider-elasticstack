...
func (model *enrollmentTokensModel) populateFromAPI(ctx context.Context, data []fleetapi.FleetEnrollmentApiKey) (diags diag.Diagnostics) {
	model.Tokens = utils.SliceToListType(ctx, data, getTokenType(), path.Root("tokens"), diags, newEnrollmentTokenModel)
	return
}

func newEnrollmentTokenModel(data fleetapi.FleetEnrollmentApiKey, meta utils.ListMeta) enrollmentTokenModel {
